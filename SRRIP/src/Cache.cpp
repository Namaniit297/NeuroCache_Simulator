...
const unsigned int MAX_RRPV = 3; // SRRIP: Max RRPV value
...
void Cache::updateSRRIPOnHit(int index, int blockIndex)
{
    cache[index][blockIndex].rrpv = 0; // High chance of reuse
}

int Cache::selectSRRIPVictim(int index)
{
    while (true)
    {
        ...
        for (unsigned long long int i = 0; i < blocksPerSet; i++)
        {
            cache[index][i].rrpv = std::min(cache[index][i].rrpv + 1, MAX_RRPV);
        }
    }
}
                 ...
                 unsigned long long int instructionCount, long long int & /*totalCycles*/)
{
    ...
    for (unsigned long long int i = 0; i < blocksPerSet; i++)
    {
        if (cache[index][i].valid && cache[index][i].tag == tag)
        {
            if (replacementPolicy == "lru")
                updateLRU(index, i);
            else if (replacementPolicy == "srrip")
                updateSRRIPOnHit(index, i);
            return true;
        }
    }
    ...
    {
        victim = selectSRRIPVictim(index);
    }
    else
    {
        // Default FIFO
    ...
    cache[index][victim].rrpv = MAX_RRPV - 1;
...
void Cache::printL1Contents()
{
    ...
    for (unsigned long long int setIdx = 0; setIdx < setsNum; setIdx++)
    {
        ...
        // Sort based on policy for viewing purposes
        if (replacementPolicy == "lru")
        {
        ...
        else if (replacementPolicy == "srrip")
        {
            std::sort(tempSet.begin(), tempSet.end(),
                      [](const CacheConstruct &a, const CacheConstruct &b) {
                          return a.rrpv < b.rrpv; // Lower RRPV = more recently used
                      });
        }
      ...
      replacementPolicy(std::move(replacement_policy))
{
    ...
    for (unsigned long long int i = 0; i < setsNum; i++)
    {
        ...
        for (unsigned long long int w = 0; w < blocksPerSet; w++)
        {
            ...
            cache[i][w].rrpv = MAX_RRPV; // Initialize RRPV to max
        }
    }
}
